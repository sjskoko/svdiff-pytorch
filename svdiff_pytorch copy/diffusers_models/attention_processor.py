...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if norm_num_groups is not None:
            self.group_norm = SVDGroupNorm(num_channels=query_dim, num_groups=norm_num_groups, eps=eps, affine=True)
        else:
        ...
        if cross_attention_norm is None:
            ...
            self.norm_cross = SVDLayerNorm(self.cross_attention_dim)
    ...
    def norm_encoder_hidden_states(self, encoder_hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if isinstance(self.norm_cross, SVDLayerNorm):
            encoder_hidden_states = self.norm_cross(encoder_hidden_states)
        elif isinstance(self.norm_cross, SVDGroupNorm):
...
class CustomDiffusionAttnProcessor(nn.Module):
    ...
    def __init__(
        ...
        if self.train_kv:
            self.to_k_custom_diffusion = SVDLinear(cross_attention_dim or hidden_size, hidden_size, bias=False)
            self.to_v_custom_diffusion = SVDLinear(cross_attention_dim or hidden_size, hidden_size, bias=False)
        if self.train_q_out:
            self.to_q_custom_diffusion = SVDLinear(hidden_size, hidden_size, bias=False)
            self.to_out_custom_diffusion = nn.ModuleList([])
            self.to_out_custom_diffusion.append(SVDLinear(hidden_size, hidden_size, bias=out_bias))
...
class SpatialNorm(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.norm_layer = SVDGroupNorm(num_channels=f_channels, num_groups=32, eps=1e-6, affine=True)
        self.conv_y = SVDConv2d(zq_channels, f_channels, kernel_size=1, stride=1, padding=0)
        self.conv_b = SVDConv2d(zq_channels, f_channels, kernel_size=1, stride=1, padding=0)
...
class IPAdapterAttnProcessor(nn.Module):
    ...
    def __init__(self, hidden_size, cross_attention_dim=None, num_tokens=4, scale=1.0):
        ...
        self.to_k_ip = SVDLinear(cross_attention_dim or hidden_size, hidden_size, bias=False)
        self.to_v_ip = SVDLinear(cross_attention_dim or hidden_size, hidden_size, bias=False)
