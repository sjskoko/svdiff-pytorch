...
from svdiff_pytorch.diffusers_models.attention_processor import SpatialNorm
from diffusers.models.lora import LoRACompatibleConv, LoRACompatibleLinear
from svdiff_pytorch.diffusers_models.normalization import AdaGroupNorm
...
class Upsample1D(nn.Module):
    ...
    def __init__(
        ...
        if use_conv_transpose:
            ...
            self.conv = SVDConv1d(self.channels, self.out_channels, 3, padding=1)
...
class Upsample2D(nn.Module):
    ...
    def __init__(
        ...
        self.use_conv_transpose = use_conv_transpose
        self.name = name
        conv_cls = SVDConv2d if USE_PEFT_BACKEND else LoRACompatibleConv
...
class TemporalConvLayer(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Sequential(
            SVDGroupNorm(norm_num_groups, in_dim),
            nn.SiLU(),
            nn.Conv3d(in_dim, out_dim, (3, 1, 1), padding=(1, 0, 0)),
        )
        self.conv2 = nn.Sequential(
            SVDGroupNorm(norm_num_groups, out_dim),
            nn.SiLU(),
            nn.Dropout(dropout),
...
class TemporalResnetBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm1 = SVDGroupNorm(num_groups=32, num_channels=in_channels, eps=eps, affine=True)
        ...
        self.norm2 = SVDGroupNorm(num_groups=32, num_channels=out_channels, eps=eps, affine=True)
